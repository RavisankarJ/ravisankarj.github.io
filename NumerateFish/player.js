export class Player{
    ...
    update(mouse, deltaTime) {
        ...
        if (this.x >= this.game.input.mouse.x){
            if(this.bubbles.length>0) this.image = this.image = this.playerLeftOpen;
            else this.image = this.playerLeft;
        }else {
            if(this.bubbles.length>0) this.image = this.image = this.playerRightOpen;
    ...
    draw() {
        ...
        if(this.bubbles){
            ...
            // this.game.ctx.fill();
            // this.game.ctx.closePath();
            this.game.ctx.drawImage(this.bubbleImage, 0-125, 0-40, this.bubbles[i].radius*2.2, this.bubbles[i].radius*2.2);
            this.game.ctx.fillStyle = '#ffffff';
            this.game.ctx.font = "bold 32px serif";
            this.game.ctx.textAlign = 'center';
            this.game.ctx.textBaseline = 'middle';
            this.game.ctx.fillText(this.bubbles[i].value,0-75, 5);
