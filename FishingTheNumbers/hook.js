...
export class Hook{
    constructor(game){
        ...
        this.audio = new Audio('assets/water_splash.ogg');
    ...
    draw() {
        ...
        if (this.distanceToFish>this.radius/2) {
            ...
            if(Math.abs(this.y-this.game.background.splashPoint.hookY)<=Math.abs(this.y - this.game.input.mouse.y)/10) {
                if(!WaterSplash.checkItHasWaterSplash(this.game.collisions))
                    {
                        this.game.collisions.push(new WaterSplash(this.game, this.x-41, this.y));
                        this.audio.play();
                    }
